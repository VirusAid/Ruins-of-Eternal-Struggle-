                   ...
                   const std::string &text )
{
    ...
    if( !catacurses::stdscr ) {
        ...
        if( excess_repetition ) {
            ...
            std::string rep_err =
                "Excessive error repetition detected.  Please file a bug report at https://github.com/VirusAid/Ruins-of-Eternal-Struggle-/issues\n            "
                + text;
    ...
    switch( output_mode ) {
        ...
        case DebugOutput::file: {
            ...
            *file << get_time() << " : Starting log.";
            DebugLog( D_INFO, D_MAIN ) << "Ruins of Eternal Struggle version " << getVersionString();
            if( rename_failed ) {
