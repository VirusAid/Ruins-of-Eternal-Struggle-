...
void basecamp::player_eats_meal()
{
    ...
    smenu.addentry( i++, true, '3', _( "Feast" ) );
    smenu.query();
    ...
    if( food_available <= 0 ) {
        popup( _( "You check storage for some food, but there isn't any." ) );
        return;
    } else if( food_available <= kcal_to_eat ) {
        add_msg( _( "There's only one meal left, so that's what you'll have." ) );
...
{
    if( must_feed &&
        fac()->food_supply().kcal() < time_to_food( duration, exertion_level ) ) {
                              ...
                              const std::string &skill, int difficulty, const bool cancel )
{
    ...
    if( !fixed_time ) {
        ...
        need_food = time_to_food( mission_time - reserve_time );
    }
...
bool basecamp::distribute_food( bool player_command )
{
    ...
    if( player_command ) {
        if( nutrients_to_add.kcal() > 0 ) {
            ...
            popup( _( "You distribute supplies to your companions." ) );
        }
    }
