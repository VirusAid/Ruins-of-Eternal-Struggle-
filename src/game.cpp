                               ...
                               const std::string &area_name, int column, int &line )
{
    ...
    if( here.concealment( lp ) > 0 && here.concealment( lp ) < 100 ) {
        int height = get_player_character().eye_level();
        if( height < here.concealment( lp ) ) {
    ...
    if( here.coverage( lp ) > 0 && here.coverage( lp ) < 100 ) {
                      ...
                      const bool furniture_move )
{
    ...
    bool cramped = false;
    if( vp_there && !u.can_move_to_vehicle_tile( get_map().get_abs( dest_loc ), cramped ) ) {
    ...
    bool diag = pos.x() != dest_loc.x() && pos.y() != dest_loc.y();
    ...
    if( u.grab_1.victim.get() ) {
        tripoint_abs_ms old_abs_ms_pos = m.get_abs( oldpos );
...
void game::vertical_move( int movez, bool force, bool peeking )
{
    ...
    if( !wall_cling && ( get_map().is_open_air( u.pos_bub() ) &&
